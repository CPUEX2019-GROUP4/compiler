...
load_label r label = do
    let r' = reg r
    liftIO $ printf "    lui %s ha(%s)\n" r' label
    liftIO $ printf "    ori %s %s lo16(%s)\n" r' r' label
...
g' xx@(NonTail x, exp)
    ...
    | Li i      <- exp, -32768 <= i, i < 32768 =
            liftIO $ printf "    addi %s r0 %d\n" (reg x) i
    | Li i      <- exp = do
            let n = i `div` (1 `shift` 16)
            let m = i `mod` (1 `shift` 16)
            let r = reg x
            liftIO $ printf "    lui %s %d\n" r n
            liftIO $ printf "    ori %s %s %d\n" r r m
    | FLi f     <- exp = return () -- C 使わなあかんのか...
    | SetL (L y)<- exp = load_label x y
    | Mv y      <- exp, x == y = return ()
    | Mv y      <- exp =
            liftIO $ printf "    mv %s %s\n" (reg x) (reg y)
    | Arith2 Add y (V z) <- exp =
            liftIO $ printf "    add %s %s %s\n" (reg x) (reg y) (reg z)
    | Arith2 Add y (C z) <- exp =
            liftIO $ printf "    addi %s %s %d\n" (reg x) (reg y) z
    | Arith2 Sub y (V z) <- exp =
            liftIO $ printf "    sub %s %s %s\n" (reg x) (reg y) (reg z)
    | Arith2 Sub y (C z) <- exp =
            liftIO $ printf "    subi %s %s %d\n" (reg x) (reg y) z
    | Slw y (V z) <- exp =
            liftIO $ printf "    sllv %s %s %s\n" (reg x) (reg y) (reg z)
    | Slw y (C z) <- exp =
            liftIO $ printf "    sll %s %s %d\n" (reg x) (reg y) z
    | Lw y (V z) <- exp = do
            liftIO $ printf "    add %s %s %s\n" (reg reg_tmp) (reg y) (reg z)
            liftIO $ printf "    lw %s %s 0\n" (reg x) (reg reg_tmp)
    | Lw y (C z) <- exp =
            liftIO $ printf "    lw %s %s %d\n" (reg x) (reg y) z
    | Sw a y (V z) <- exp = do
            liftIO $ printf "    add %s %s %s\n" (reg reg_tmp) (reg y) (reg z)
            liftIO $ printf "    sw %s %s 0\n" (reg a) (reg reg_tmp)
    | Sw a y (C z) <- exp =
            liftIO $ printf "    sw %s %s 0\n" (reg a) (reg y) z
    | FMv y      <- exp, x == y = return ()
    | FMv y      <- exp =
            liftIO $ printf "    fmv %s %s\n" (reg x) (reg y)
    | Cmp Syntax.Eq y (V z) <- exp =
            liftIO $ printf "    sub %s %s %s\n" (reg x) (reg y) (reg z)
    | Cmp Syntax.Eq y (C z) <- exp =
            liftIO $ printf "    subi %s %s %s\n" (reg x) (reg y) z
    | Lf y (V z) <- exp = do
            liftIO $ printf "    add %s %s %s\n" (reg_tmp) (reg y) (reg z)
            liftIO $ printf "    lwcZ %s %s 0\n" (reg x) (reg_tmp)
    | Lf y (C z) <- exp = do
            liftIO $ printf "    lwcZ %s %s %d\n" (reg x) (reg y) z
    | Sf a y (V z) <- exp = do
            liftIO $ printf "    add %s %s %s\n" (reg reg_tmp) (reg y) (reg z)
            liftIO $ printf "    swcZ %s %s 0\n" (reg a) (reg reg_tmp)
    | Sf a y (C z) <- exp =
            liftIO $ printf "    swcZ %s %s 0\n" (reg a) (reg y) z
    | Save a b  <- exp = g'_stackset xx =<< (stackset <$> get)
    ...
    | Restore y <- exp, elem x allfregs = do
            ofset <- offset y
            liftIO $ printf "    lwcZ %s %s %d\n" (reg x) (reg reg_sp) ofset
    | Restore y <- exp = throw $ Fail "my god ..."
g' (Tail, exp)
    | Nop           <- exp = e
    | Sw _ _ _      <- exp = e
    | Sf _ _ _      <- exp = e
...
g' (Tail, exp)
    ...
    where e = do
            ...
            liftIO $ printf "    jr %s\n" (reg reg_lr)
g' (Tail, restore) = throw $ Fail "restore at tail !?!?"
...
g'_stackset :: (Dest, Exp) -> Set String -> RunRun ()
g'_stackset (NonTail _, Save x y) stset
    | elem x allregs, tf    = do
            save y
            ofset <- offset y
            liftIO $ printf "    sw %s %s %d\n" (reg x) (reg reg_sp) ofset
    | elem x allfregs, tf   = do
            save y
            ofset <- offset y
            liftIO $ printf "    swcZ %s %s %d\n" (reg x) (reg reg_sp) ofset
    | not tf                = return ()
    | otherwise             = throw $ Fail "maji!?"
    where
        tf = notMember y stset
g'_stackset _ _ = throw $ Fail "e?"


f :: Aprog -> RunRun ()
f (Aprog fundefs e) = do
    liftIO $ putStrLn "emit ..."
    liftIO $ print e
    liftIO $ putStrLn "assembly"
    liftIO $ printf "    ori %s r0 3072\n" (reg reg_sp)
    liftIO $ printf "    ori %s r0 10240\n" (reg reg_hp)
    f <- get
    put $ f { stackset = empty, stackmap = [] }
    g (NonTail("_r0"), e)
    liftIO $ putStrLn "end_of_program:\n    nop\n    beq r0 r0 end_of_program\n"
