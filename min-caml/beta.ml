...
let rec g env = function (* β簡約ルーチン本体 (caml2html: beta_g) *)
  ...
  | Mul4(x) -> Mul4(find x env)
  | Mul10(x) -> Mul10(find x env)
  | Div2(x) -> Div2(find x env)
  | Div10(x) -> Div10(find x env)
  ...
  | IfFLt(x, y, e1, e2) -> IfFLt(find x env, find y env, g env e1, g env e2)
  | IfFZero(x, e1, e2) -> IfFZero(find x env, g env e1, g env e2)
